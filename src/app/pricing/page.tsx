...
const InteractiveCalculator = () => {
    ...
    const totalPrice = useMemo(() => {
        ...
        if (services.seo) {
            seoPrice = seoKeywords * 3 + seoBacklinks * 30;
        }
        if (services.pwa) {
            pwaPrice = pwaType === 1 ? 999 : 2499;
        }

        if(services.seo && services.pwa) {
            priceText = `od ${pwaPrice} € jednorazovo + ${seoPrice} € / mesiac`;
        } else if (services.seo) {
            priceText = `${seoPrice} € / mesiac`;
        } else if (services.pwa) {
            priceText = `od ${pwaPrice} € jednorazovo`;
        }
                ...
                <motion.div variants={itemVariants} className="lg:sticky top-24 h-fit">
                    <Card className="bg-card shadow-2xl shadow-primary/10">
                        ...
                        <CardContent className="text-center">
                            <p className="text-3xl md:text-4xl font-bold text-primary min-h-[64px] flex items-center justify-center text-balance">
